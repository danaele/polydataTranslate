    ...
    if( j != StringToTestFunctionMap.end() )
    {
        ...
        try
        {
            ...
            for( int i = 0 ; i < static_cast< int >( compareList.size() ) ; i++ )
            {
                ...
                std::cout << vtksys::SystemTools::GetFilenameName( bestBaseline ) ;
                           ...
                           )
{
    ...
    if( ReadVTKVTPFile( testImageFilename , vtksys::SystemTools::GetFilenameExtension( testImageFilename ) , vtkpolydata1 ) != 0 )
    {
        return -1 ;
    }
    if( ReadVTKVTPFile( baselineImageFilename , vtksys::SystemTools::GetFilenameExtension( baselineImageFilename) , vtkpolydata2 ) !=0 )
    {
            return -1 ;
    }
